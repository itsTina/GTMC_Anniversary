  ...
  Zoomage.prototype = {
    ...
    _enableGestureRotate: function(touches) {
      ...
      if (touches.length >= 2) {
        ...
        var rotateAngel = Math.atan2(y*2, x) * 180 / Math.PI;
      }
    ...
    _setEventListeners_Transform: function() {
      ...
      this.container.addEventListener('touchmove', function(e) {
        ...
        if (e.touches.length == 2) {
          ...
          // Zoom
          this.zoomD = this.zoomD + this._gesturePinchZoom(e.touches) / 2000;
          ...
          if (this.lastX !== null && this.lastY !== null) {
            this.moveXD += (e.touches[0].pageX - this.lastX)/2;
            this.moveYD += (e.touches[0].pageY - this.lastY)/2;
          }
      ...
      this.imgTexture.addEventListener('load', function(e) {
        ...
        EXIF.getData(this.imgTexture, function () {
          var iw = _this.imgTexture.naturalWidth;
          var ih = _this.imgTexture.naturalHeight;
          ...
          if(iw> 4000 || ih>4000){
            _this.minZoom = 0.04;
            console.log('minZoom:'+_this.minZoom);
          }
          console.log('width:'+iw+';height:'+ih);
          if (navigator.userAgent.match(/iphone/gi) && ( Orientation && Orientation!=1)) {
            ...
            if (iw > ih && iw > 800) {
              expectWidth = 800;
              expectHeight = expectWidth * ih / iw;
            } else if (ih > iw && ih > 1200) {
              expectHeight = 1200;
              expectWidth = expectHeight * iw / ih;
            }
            var canvas = _this.canvas;
            var ctx = _this.context;
            canvas.width = expectWidth;
            canvas.height = expectHeight;
            ctx.drawImage(_this.imgTexture, 0, 0, expectWidth, expectHeight);
            if(Orientation && Orientation != 1){
              switch(Orientation){
                case 6:
                  _this.rotateImg(_this.imgTexture,'left',canvas);
                  break;
                case 8:
                  _this.rotateImg(_this.imgTexture,'right',canvas);
                  break;
                case 3://需要180度旋转

                  _this.rotateImg(_this.imgTexture,'right',canvas);//转两次
                  _this.rotateImg(_this.imgTexture,'right',canvas);
                  break;
              }
            }
          ...
          _this.moveXD = 0;
          _this.rotate.angle = 0;

          _this.moveYD = Math.abs( document.body.offsetHeight - _this.canvas.height * _this.zoomD) * 0.4;

          console.log('moveYD:'+_this.moveYD);
          console.log('zoomD:'+_this.zoomD);
          _this._animate_Transform();
          //return;
        });
      }.bind(this));
